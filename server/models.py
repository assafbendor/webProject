...
class Copy(Base):
    ...
    borrows = relationship('Borrow', back_populates='copy', lazy='joined')
    is_borrowed = Column(Boolean, default=False)

    ordered_by_email = Column(String, ForeignKey('readers.email'), default=None)
    ordered_by = relationship('Reader', back_populates='ordered_copies', lazy='joined')

    waiting = relationship('Waiting', back_populates='copy', lazy='joined')

    def __repr__(self):
        return f"<Copy(id={self.id}, book={self.book}, is_borrowed={self.is_borrowed}, ordered_by={self.ordered_by})>"
